...
import { RocketAction } from './utils/RocketAction';
...
export function rocketReducer(state = initialState, action): Rocket[] {
  const RESERVE_ROCKET = 'spacetravelers/rockets/reserve_rocket';
  switch (action.type) {
    case RESERVE_ROCKET:
      return state.map(rocket => rocket['rocket_id'] === action.payload ? { ...rocket, reserved: !rocket['reserved'] } : rocket)
    default:
      return state;
  }
