...
export function missionReducer(state = initialState, action): Missions[] {
  const JOIN_MISSION = 'spacetravelers/missions/join_mission';
  switch (action.type) {
    case JOIN_MISSION:
      return state.map(mission => mission['mission_id'] === action.payload ? { ...mission, member: !mission['member'] } : mission)
    default:
      return state;
  }
